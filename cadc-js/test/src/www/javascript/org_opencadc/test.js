test('Test String util.', 6, function() {
  ...
  equal(
    stringUtil.format('Val {1} is {2} but not {3}', ['ONE', 'TWO']),
...
test('String util contains', 5, function() {
  ...
  equal(
    testSubject.contains(testString, 'BEEL', false),
    true,
    'Should contain BEEL'
  )
  equal(
    testSubject.contains(testString, 'belong'),
    false,
    'Should not contain belong'
  )
  equal(testSubject.contains(testString, '!'), true, 'Should contain !')
  equal(testSubject.contains(testString, 'BAse'), true, 'Should contain BAse')
  equal(
    testSubject.contains(testString, 'are', true),
    false,
    'Should not contain are'
  )
})
