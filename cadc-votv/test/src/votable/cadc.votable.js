...
test('Test table functions.', 2, function() {
  new cadc.vot.Builder(
    ...
    function(voTableBuilder) {
      ...
      equal(
        memoryField.containsInterval(),
        false,
        'Memory field should contain no INTERVAL'
      )

      equal(
        jobStartsField.containsInterval(),
        true,
      ...
      function(voTableBuilder) {
        ...
        for (var r in resources) {
          ...
          for (var t in tables) {
            ...
            equal(
              tableData.getLongestValues()['Command'],
              3,
              'Longest value for Command should be 3'
            )
            equal(
              tableData.getLongestValues()['VM Type'],
              10,
              'Longest value for VM Type should be 10'
            )
          }
        }
      },
