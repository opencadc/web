  ...
  function Viewer(targetNodeSelector, options) {
    ...
    function build(input, completeCallback, errorCallBack) {
      ...
      new cadc.vot.Builder(
        getOptions().maxRowLimit,
        input,
        function(voTableBuilder) {
          voTableBuilder.subscribe(cadc.vot.onDataLoadComplete, function(
            event,
            args
          ) {
            ...
            if (input.xmlDOM) {
              ...
              load(args.builder.getVOTable(), false, true)
            }
          ...
          var hasDisplayColumns =
            _self.displayColumns && _self.displayColumns.length > 0
    ...
    function valueFilters(filter, value) {
      ...
      if (dotIndex > 0) {
        ...
        var match =
          matches !== null && matches.length > 1 ? $.trim(matches[1]) : null
    ...
    function calculateColumnWidth(_column) {
      ...
      var textWidthToUse =
        longestCalculatedWidth > minWidth ? longestCalculatedWidth : minWidth
    ...
    function setupHeader(checkboxSelector, args) {
      ...
      if (
        ...
        var tooltipTitle = datatype.isNumeric()
    ...
    function init() {
      var forceFitMax =
        getColumnManager().forceFitColumns &&
        getColumnManager().forceFitColumnMode &&
        getColumnManager().forceFitColumnMode == 'max'
      var checkboxSelector
      var enableSelection =
        !getOptions().enableSelection || getOptions().enableSelection == true
      ...
      if (checkboxSelector && enableSelection) {
        ...
        var colsToCheck =
          getDisplayColumns().length == 0 ? getColumns() : getDisplayColumns()
      ...
      getOptions().defaultFormatter = function(
        row,
        cell,
        value,
        columnDef,
      ...
      dataView.onRowCountChanged.subscribe(function(e, args) {
        trigger(cadc.vot.events.onRowsChanged, args)
      })
      ...
      $('#inlineFilterPanel')
        .appendTo(_self.grid.getTopPanel())
        .show()
